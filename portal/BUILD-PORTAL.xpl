...
<p:declare-step xmlns:p="http://www.w3.org/ns/xproc" version="3.0"
  ...
  <p:for-each name="iteration">
    <p:with-input select="/">
      ...
      <p:document href="xproc-lab/xproc-from-above.xhtml" content-type="application/xml"/>
    </p:with-input>
    ...
    <p:variable name="filedir"
      select="((tokenize($filepath,'/')[not(position() eq last())] => string-join('/'))[normalize-space(.)],'.')[1]"/>
    <!-- $pagename is the base name of the file, w/o suffix 'xhtml' -->
    <p:variable name="pagename"
      select="tokenize( $filepath,'/' )[last()] => replace('\.[^\.]+$','')"/>
    <p:variable name="path-to-root"
      select="'.' || (tokenize($filepath,'/')[not(position() eq 1)] ! '/..' )"/>
    <p:variable name="target-path"
      select="($pubdir, $filedir, ($pagename || '.html')) => string-join('/')"/>
